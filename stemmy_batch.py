...
def main():
    ...
    ap.add_argument("--format-id", default=None,
                    help="Bestaand format UUID (anders: auto-aanmaken)")
    ...
    for i, ep in enumerate(episodes, 1):
        ...
        title = ep["title"]
        print(f"\n[{i}/{len(episodes)}] {title[:65]}", flush=True)

        # Dedupe check
        item_id, existing_audio_url = item_exists(format_id, source_url)
        if item_id and has_fragments(item_id):
            print(f"  ↩ Al getranscribeerd, skip", flush=True)
            stats["skipped"] += 1
            continue

        # Nieuw item of bestaand item zonder fragments
        if not item_id:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                # Stap 4: Item naar Turso (voor transcriptie start)
                save_item(item_id, format_id, ep, source_url, audio_url)
